...
2.  Create a new column in your dataset called "Percent\_ pos" and set it equal to (number of positive samples / number of samples)\*100.
...
rabies %>% 
  ...
  summarise(
    num_received = sum(Number.of.Samples)) 
```
...
rabies %>% 
  ...
  summarise(
    pos_submissions = sum(Positive.Samples)) %>%
View()
```
...
```{r}
rabies %>% 
  select(Animal.Classification,Percent_pos, Number.of.Samples) %>% 
...
SLC_animals <- SLC %>% 
  group_by(Animal.Classification) %>% 
  summarise(
    total_pos = sum(Positive.Samples),
    sub_total = sum(Number.of.Samples),
    total_pos_percent = (total_pos/sub_total)*100) %>% 
View()
```
...
```{r}
SLC_recent <- rabies %>% 
  filter(County.Name == "St. Lawrence", Received.Year > 2019)
```
...
```{r}
mean_slc <- SLC %>%
  group_by(Animal.Classification, Received.Year) %>%
  summarise(
    percent_high = max(Percent_pos),
    percent_mean = mean(Percent_pos))
```
