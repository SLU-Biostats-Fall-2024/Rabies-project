...
```{r}
read.csv(here("data", "Rabies_Laboratory_Submissions.csv"))
...
rabies %>% 
  ...
  summarise(num.recieved = sum(Number.of.Samples)) 
## 2008 had the most number of sample submissions 
...
rabies %>% 
  ...
  summarise(positive.num.samples = sum(Positive.Samples)) %>% 
  arrange(positive.num.samples)
  view()

## 2008 in Tompkins was the highest number of positive samples
