...
```{r}
rabies |> mutate(Percent_pos = (Positive.Samples/Number.of.Samples)*100)
```
...
```{r}
bat_pos <- rabies |> 
  filter(Animal.Classification == "Bat", Positive.Samples >=1)
```
...
SLC_animals <- SLC |> group_by(Received.Year, Animal.Classification) |>
  summarise(
    total_year = Number.of.Samples,
    pos_year = Positive.Samples,
    perc_pos = (pos_year/total_year) * 100
  )
```
...
```{r}
SLC_recent <- rabies |> 
  filter(County.Name == "St. Lawrence", Received.Year >= 2020)
```
...
```{r}
SLC_animals |> arrange(desc(perc_pos))
# bat from 2009
SLC_animals |> group_by(Received.Year) |>
  summarise(avg_perc_pos = mean(perc_pos))
```
